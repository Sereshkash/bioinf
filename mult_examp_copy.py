...
gen_seq_S = gen_seq.generation_seq(10**7, 10**3, 400, x = 1.5, alphabet = 'GTAC')
...
def mult_iteration(j):
    ...
    sq.nhmmer_on_stock_msa(name_set_subseq1_msa_stockh, s_name_fasta, name_result_nhmmer_info_1, name_result_nhmmer_posision, 1)
    sq.creating_fasta_new_nhmmer_set(s_name_txt, name_result_nhmmer_posision, N, name_set_subseq0)
    sq.creating_txt_res_nhmmer_set(s_name_txt, name_result_nhmmer_posision, min_len_subseq = 300, max_len_subseq = 500, N =  10 ** 6, name_res_txt = name_set_res)
    for i in range(20):
        #sq.creating_fasta_set(s_name_txt, N, len_subseq, name_set_subseq0)
        f1 = open(name_set_res, 'r')
        prev_set_subseq0 = f1.readlines()
        len_prev_subseq = len(prev_set_subseq0)
        f1.close()
        ...
        len_present_subseq = len(f2.readlines())
        f2.close()
...
if __name__ == '__main__':
    ...
    print('N = {}, index_N = {}, time = {} min'.format(max_N, max_N_index,round(time_res / 60, 2)))
